...
//Original source from: https://github.com/jakepusateri/graphql-list-fields
    ...
    return selections.reduce((set, ast) => {
        ...
        switch (ast.kind) {
            case 'Field':
                ...
                if (ast.selectionSet && prefix !== 'items') {
                    return Object.assign({}, set, getFieldSet(context, ast, newPrefix));
                } else if (ast.name.value === 'items') {
                    return Object.assign({}, set, getFieldSet(context, ast, 'items'));
                } else {
                    set[newPrefix] = true;
                    return set;
                }
